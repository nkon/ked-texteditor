...
テストで良く使われる`assert_eq!`や`assert!`マクロは通常の文脈でも使うことができる。しかしRustの場合はコンパイラが厳格なので、Cであれば`assert!`でチェックするようなところは、コンパイラが警告してくれていることが多い、という印象だ。たとえば、配列のオーバーランなどは、assertを入れておかなくても、変に暴走せず即座に実行時エラーとなる。
